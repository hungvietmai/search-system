...
async def search_similar_leaves(
    ...
    use_two_stage: bool = Query(False, description="Use two-stage search with re-ranking"),
    adaptive_rerank: bool = Query(False, description="Use adaptive re-ranking"),
    prefer_source: str = Query("lab", regex="^(lab|field)$", description="Preferred image source"),
    promote_diversity: bool = Query(True, description="Promote species diversity in results"),
    explain_results: bool = Query(False, description="Include explanations and confidence scores"),
    use_query_preprocessing: bool = Query(False, description="Enable query image enhancement and normalization - WARNING: Dataset was indexed WITHOUT preprocessing, enabling this may cause feature space mismatch and poor results!"),
    similarity_metric: Optional[SimilarityMetricType] = Query(None, description="Similarity metric to use: cosine, inner_product, l2, l1, angular (NEW!)"),
    db: Session = Depends(get_db)
):
    """
    Search for similar leaf images with advanced features
    
    **IMPORTANT - Preprocessing**:
    - **use_query_preprocessing**: Default is FALSE to match dataset indexing
    - Dataset images were indexed WITHOUT preprocessing (raw features)
    - Enabling preprocessing creates feature space mismatch → poor results
    - Only enable if you re-indexed the dataset WITH preprocessing
    
    **Latest Features** (NEW!):
    - **similarity_metric**: Choose metric (cosine, inner_product, l2, l1, angular)
    
    **Advanced Features**:
    - **explain_results**: Get AI-powered explanations for why each result matches
    ...
    **Core Features**:
    ...
    - **search_engine**: Engine to use ('milvus' or 'faiss')
    - **use_segmented**: Whether to return segmented images
    - **use_two_stage**: Enable two-stage search with re-ranking
    - **adaptive_rerank**: Use adaptive re-ranking weights
    - **prefer_source**: Prefer 'lab' or 'field' images (default: 'lab')
    - **promote_diversity**: Promote species diversity in results (default: True)
    """
    temp_file = None
    ...
    try:
        ...
        # Convert similarity metric from schema enum to SimilarityMetric if provided
        search_metric = None
        if similarity_metric:
            search_metric = SimilarityMetric(similarity_metric.value)
            logger.info(f"Using custom similarity metric: {search_metric.value}")
        
        # Check search cache first if enabled
        search_cache = get_search_cache() if settings.enable_search_caching else None
        cached_results = None
        file_ids = []
        distances = []
        
        # Extract features with query preprocessing if enabled
        feature_extractor = get_feature_extractor(use_query_preprocessing=use_query_preprocessing)
        query_features = feature_extractor.extract_features(temp_file, is_query=use_query_preprocessing)
        
        if use_query_preprocessing:
            logger.warning("⚠️  Query preprocessing ENABLED - This may cause poor results if dataset was indexed without preprocessing!")
            logger.info("Query preprocessing applied: enhancement + normalization + denoising")
        else:
            logger.info("✓ Query preprocessing DISABLED (matching dataset indexing)")
        
        # Try to get cached search results
        if search_cache and settings.enable_search_caching:
            cached_results = search_cache.get_results(
                query_features,
                top_k,
                search_engine.value
            )
            if cached_results:
                file_ids, distances = cached_results
                logger.info(f"Cache HIT: Retrieved {len(file_ids)} results from cache")
        
        # Choose search method
        if cached_results is None and use_two_stage:
            # Two-stage search with re-ranking
            logger.info(f"Using two-stage search (adaptive={adaptive_rerank})")
            
            # Get re-ranker
            reranker = get_two_stage_engine(adaptive=adaptive_rerank)
            
            # Define search function based on engine
            if search_engine == SearchEngine.MILVUS:
                ...
                search_func = lambda feat, k: milvus_client.search(feat, k, metric=search_metric)
            else:  # FAISS
                faiss_client = get_faiss_client()
                if not faiss_client or not faiss_client.loaded:
                    raise HTTPException(status_code=503, detail="Faiss service not available")
                search_func = lambda feat, k: faiss_client.search(feat, k, metric=search_metric)
            
            # Perform two-stage search
            candidates = reranker.search(
            ...
            for candidate in candidates:
                results.append(SearchResult(
                    ...
                    image_path=candidate.segmented_path if use_segmented and candidate.segmented_path else candidate.image_path,
            ...
            # Standard single-stage search
            if search_engine == SearchEngine.MILVUS:
                milvus_client = get_milvus_client()
                if not milvus_client or not milvus_client.connected:
                    raise HTTPException(status_code=503, detail="Milvus service not available")
                
                file_ids, distances = milvus_client.search(query_features, top_k, metric=search_metric)
            else:  # FAISS
                faiss_client = get_faiss_client()
                if not faiss_client or not faiss_client.loaded:
                    raise HTTPException(status_code=503, detail="Faiss service not available")
            ...
            if search_cache and settings.enable_search_caching:
                ...
                logger.info(f"Cached {len(file_ids)} search results")
        
        # Process results with optimized database queries
        if settings.enable_query_optimization:
            # Use bulk fetch for better performance
            optimizer = get_performance_optimizer(OptimizationConfig(
            ...
            optimized_results = optimizer.optimize_search(file_ids, distances, db, top_k)
            
            # Build results from optimized fetch
            results = []
            for opt_result in optimized_results:
                image = opt_result['image']
                distance = opt_result['distance']
            ...
            # Original method: one query per result (slower)
            results = []
        ...
        if explain_results and results:
            ...
            # Convert results to dict format for explainer
            result_dicts = [
                {
                    'file_id': r.file_id,
                    'species': r.species,
        ...
        # Determine which metric was actually used
        actual_metric = search_metric.value if search_metric else "l2"
        ...
        return SearchResponse(
            ...
            query_preprocessing_applied=use_query_preprocessing,
            similarity_metric=actual_metric
        )
