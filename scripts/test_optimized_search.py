"""
Test script to verify the optimized search endpoint with trained LTR model
"""
import requests
import os
...
def test_optimized_search():
    """
    Test the optimized search endpoint
    """
    # URL for the optimized search endpoint
    ...
    with open(test_image_path, 'rb') as image_file:
        ...
        try:
            print("Sending request to optimized search endpoint...")
            response = requests.post(url, files=files, data=data)
            
            if response.status_code == 200:
                print("✓ Optimized search endpoint is working correctly!")
            ...
            print("✗ Cannot connect to the server. Make sure the FastAPI app is running on localhost:8000")
            return False
...
if __name__ == "__main__":
    print("Testing Optimized Search Integration")
    print("="*40)
    ...
    endpoint_works = test_optimized_search()
    print()
    
    if model_exists and endpoint_works:
        print("✓ All tests passed! The optimized search with LTR model is properly integrated.")
